...
print pic_set.shape;
pca = PCA(n_components = ncomponent,svd_solver='randomized').fit(pic_set)
pic_set_ld = pca.transform(pic_set)
print pic_set_ld.shape
...
def dist(u,v):
	...
	return np.dot(u,v)
test_face_ld = pca.transform(test_face)
result = []
for i in range(0, test_face_ld.shape[0]):
	maxdist = -float('inf')
	maxj = 0
	match_pic_ld = test_face_ld[i]
